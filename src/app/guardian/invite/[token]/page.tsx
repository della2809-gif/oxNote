...
export default async function FamilyInvitationPage({
  ...
  const next = `/guardian/invite/${token}`;
  ...
  const { data: invitation } = await admin
    ...
    .select("direction, invitee_email, child_name, relationship, status, expires_at")
  ...
  return (
    <main className="mx-auto flex min-h-screen max-w-xl items-center px-4 py-10">
      <section className="w-full rounded-2xl border border-neutral-200 bg-white p-6 shadow-sm dark:border-neutral-800 dark:bg-neutral-950">
        ...
        <h1 className="mt-8 text-2xl font-bold">보호자·자녀 연결 승인</h1>
        ...
        {!invitation ? (
          <div className="mt-6">
            <p className="rounded-lg bg-neutral-100 p-4 text-sm text-neutral-600 dark:bg-neutral-900 dark:text-neutral-300">
              초대가 만료되었거나 이미 승인되었습니다. 초대한 사람에게 새 링크를 요청해 주세요.
            </p>
          ...
          <form action={acceptFamilyInvitation} className="mt-6 space-y-5">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <label className="block text-sm">
                <span className="font-medium">새 비밀번호 <small className="text-neutral-500">(초대 가입자만)</small></span>
                <input name="password" type="password" minLength={8} autoComplete="new-password" className="mt-1 w-full rounded-md border border-neutral-300 px-3 py-2 dark:border-neutral-700 dark:bg-neutral-900" />
              </label>
              <label className="block text-sm">
                <span className="font-medium">새 비밀번호 확인</span>
                <input name="passwordConfirm" type="password" minLength={8} autoComplete="new-password" className="mt-1 w-full rounded-md border border-neutral-300 px-3 py-2 dark:border-neutral-700 dark:bg-neutral-900" />
              </label>
            </div>
