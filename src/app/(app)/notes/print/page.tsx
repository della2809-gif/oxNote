  ...
  return (
    <article className={`print-sheet ${variant === "problem" ? "print-problem-sheet" : ""}`}>
      {variant !== "problem" && <header className="flex items-start justify-between gap-4 border-b border-slate-200 pb-3">
        <div><p className="text-[9px] font-bold uppercase tracking-[0.18em] text-indigo-600">xonote · {variant === "solution" ? "해설지" : variant === "answer" ? "풀이 복습" : "AI 오답노트"}</p><div className="flex items-start gap-2"><strong className="print-question-number text-lg">{index + 1}</strong><h1 className="mt-1.5 text-base font-bold leading-tight"><MathText>{compactText(details.title || note.question, 100)}</MathText></h1></div><div className="mt-2 flex flex-wrap gap-1.5 text-[8px] font-semibold">{subject && <span className="rounded-full bg-indigo-50 px-2 py-1 text-indigo-700">{subject.name}</span>}{note.source && <span className="rounded-full bg-slate-100 px-2 py-1 text-slate-600">{compactText(note.source, 40)}</span>}</div></div>
