...
- 거듭제곱·근호 문제는 밑을 가능한 한 같은 소수의 거듭제곱으로 바꾸고 지수법칙 \\(a^m a^n=a^{m+n}\\), \\((a^m)^n=a^{mn}\\), \\(a^{-m}=1/a^m\\)을 적용한다. 분수 지수의 부호와 분모를 유지한 채 지수를 먼저 계산하고, 얻은 값을 원래 식에 수치로 대입해 검산한다.
- 수학 solution_steps의 formula에 있는 완성된 식은 \\(...\\)로 감싼 LaTeX로 쓴다. 설명 문장과 식을 섞을 때도 각 식만 구분자로 감싸며, JSON 이스케이프 외에 역슬래시를 이중으로 출력하지 않는다.
...
- formula에는 '계산식 = 중간값 = 최종값 단위'를 LaTeX 수식으로 쓴다. 정확히 같은 값에만 =를 쓰고 근삿값에는 \\approx를 쓴다.
